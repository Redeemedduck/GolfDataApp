...
def _fetch_supabase_sessions(page_size=1000):
    ...
    while True:
        query = (
            ...
            .select('session_id, date_added, session_date, session_type')
...
def get_unique_sessions(read_mode=None):
    ...
    try:
        if os.path.exists(SQLITE_DB_PATH):
            conn = sqlite3.connect(SQLITE_DB_PATH)
            query = (
                "SELECT session_id, MAX(date_added) as date_added, MAX(session_date) as session_date, MAX(session_type) as session_type "
