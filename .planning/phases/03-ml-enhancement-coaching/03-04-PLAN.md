...
must_haves:
  truths:
    ...
    - "User sees model version, training date, and metrics displayed in UI after retraining"
    - "User sees practice plan rendered with drill expanders showing name, duration, reps, and step-by-step instructions"
  artifacts:
  ...
  key_links:
    ...
    - from: "pages/4_🤖_AI_Coach.py"
      to: "local_coach.py"
      via: "practice plan visual rendering from CoachResponse.data"
      pattern: "response\\.data.*plan|st\\.expander"
---
...
<task type="auto">
  ...
  <files>pages/4_🤖_AI_Coach.py</files>
  <action>
  Modify `pages/4_🤖_AI_Coach.py` to add model management and interval prediction sections:
  ...
  5. **Wire prediction intervals into chat context**:
     ...
     - Add bullet: "**Practice Plans**: Ask 'Give me a practice plan' to get a personalized drill plan based on your data"

  6. **Render practice plan visually in chat response handler** (COACH-02 delivery):
     - In the chat response rendering section, after receiving a CoachResponse from LocalCoach:
     - Check if `response.data` contains a `'plan'` key (indicates practice plan response)
     - If practice plan detected, render structured visual layout instead of plain text:
       * `st.subheader(f"🎯 Practice Plan ({plan['duration_min']} min)")`
       * Two `st.columns([1,1])`: left column shows focus areas as `st.caption`, right column shows weaknesses addressed
       * For each drill in `plan['drills']`, use `st.expander(f"📋 {drill['name']} — {drill['duration_min']} min, {drill['reps']} reps")`:
         - Inside expander: `st.markdown(drill['instructions'])`
         - Inside expander: `st.caption(f"Focus: {drill['focus']}")`
       * Below drills: `st.info(f"💡 {plan['rationale']}")`
     - If response.data has no 'plan' key, render as normal chat text (existing behavior unchanged)
     - Import PracticePlan/Drill types with try/except guard for type hints only

  Key constraints:
  - Do NOT restructure the existing chat flow — add alongside it
  - All new features behind try/except guards (ML may not be available)
  - Retraining panel appears below chat, not replacing it
  - Practice plan rendering only activates when CoachResponse.data contains 'plan'
  - Keep the page clean — model management is secondary to chat
  </action>
  <verify>
  python -m py_compile "pages/4_🤖_AI_Coach.py"
  python -c "import importlib.util; spec = importlib.util.spec_from_file_location('coach', 'pages/4_🤖_AI_Coach.py'); print('AI Coach page compiles OK')"
  </verify>
  <done>
  ...
  - Practice plan responses render visually with drill expanders (name, duration, reps, instructions) instead of plain text
...
4. Practice plan responses render visually with drill expanders (not plain text) in AI Coach chat
