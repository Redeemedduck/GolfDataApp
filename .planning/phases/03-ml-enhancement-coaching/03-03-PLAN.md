...
must_haves:
  truths:
    ...
    - "User sees practice plan rendered visually in AI Coach chat with drill expanders showing name, duration, reps, and instructions"
  artifacts:
    - path: "local_coach.py"
      provides: "Analytics-driven LocalCoach with practice plans and interval predictions"
      contains: "get_practice_plan"
    - path: "pages/4_🤖_AI_Coach.py"
      provides: "AI Coach page with practice plan visual rendering in chat handler"
  ...
  key_links:
    ...
    - from: "pages/4_🤖_AI_Coach.py"
      to: "local_coach.py"
      via: "practice plan chat handler renders PracticePlan visually"
      pattern: "get_practice_plan|CoachResponse\\.data"
---
...
<task type="auto">
  <name>Task 1: Replace template-based club stats with analytics-driven responses</name>
  <files>local_coach.py</files>
  <action>
  Modify `local_coach.py` to evolve club stats and trends from template-based to analytics-driven:

  1. **Add analytics imports** (all guarded with try/except):
  ...
  2. **Replace `_handle_club_stats`** with analytics-driven version:
  ...
  3. **Update `_handle_trends`** to use analytics stats:
  ...
  - _handle_trends uses analytics stats
...
<task type="auto">
  <name>Task 2: Add practice plan integration, prediction intervals, and wire into AI Coach page</name>
  <files>local_coach.py, pages/4_🤖_AI_Coach.py</files>
  <action>
  Add practice plan and interval support to LocalCoach, then wire the practice plan intent into the AI Coach page's chat handler:

  1. **Add coaching imports to `local_coach.py`** (guarded with try/except):
     - `from ml.coaching import PracticePlanner, WeaknessMapper` (guarded: set COACHING_AVAILABLE flag)

  2. **Add new intent pattern** in `local_coach.py`:
  ...
  3. **Add `get_practice_plan` method** to LocalCoach:
  ...
  4. **Add `_handle_practice_plan` handler** in LocalCoach:
  ...
  5. **Integrate prediction intervals into `predict_distance`**:
  ...
  6. **Wire practice plan into AI Coach page** (`pages/4_🤖_AI_Coach.py`):
     - In the chat response handler, detect when LocalCoach returns a practice plan response (check CoachResponse.data for 'plan' key)
     - When a practice plan is returned, render it visually instead of plain text:
       * Use `st.subheader(f"Practice Plan ({plan.duration_min} min)")`
       * Use `st.caption(f"Focus: {', '.join(plan.focus_areas)}")`
       * For each drill, use `st.expander(f"{drill.name} ({drill.duration_min} min, {drill.reps} reps)")` containing `st.markdown(drill.instructions)`
       * Show rationale in `st.info(plan.rationale)`
     - This ensures the user RECEIVES the practice plan visually (COACH-02 delivery)

  Key constraints:
  - All new imports MUST be try/except guarded
  - Existing public API MUST remain backward compatible
  - CoachResponse dataclass is unchanged
  </action>
  <verify>
  python -m py_compile local_coach.py
  python -m py_compile "pages/4_🤖_AI_Coach.py"
  ...
  - AI Coach page renders practice plans visually with expanders for each drill
  - Practice plan intent triggers full visual rendering in the UI (COACH-02 delivered)
  - All imports gracefully degrade
  </done>
</task>

<task type="auto">
  <name>Task 3: Update and extend LocalCoach tests for analytics-driven behavior</name>
...
<success_criteria>
1. LocalCoach.get_response("How's my 7-iron?") returns response citing median carry, dispersion IQR, and shot shape from analytics — not generic templates
...
4. AI Coach page renders practice plans visually with drill expanders when user asks for a practice plan
5. All 207+ existing tests pass plus 10+ new tests
</success_criteria>
